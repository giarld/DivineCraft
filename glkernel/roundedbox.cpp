...
//        qWarning()<<vertexCountPerCorner<<" "<<centre<<" "<<winding<<" "<<offsX<<" "<<offsY<<" "<<offsZ;
        // Face polygons    面的多边形
        if (winding) {
            ip[iidx++] = vidx;
            ip[iidx++] = vidx + offsX;
...
//            }
            ...
            for (int j = 0; j <= i; ++j) {
                ...
                qWarning()<<vp[vidx].position;
                qWarning()<<vp[vidx].normal;
                qWarning()<<vp[vidx].texCoord;

                // Corner polygons  角的多边形
//                if (i < n + 1) {
//                    ip[iidx++] = vidx;
